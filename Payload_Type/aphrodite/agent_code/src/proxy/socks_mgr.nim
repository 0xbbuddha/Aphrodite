...
proc socketReaderProc(idx: int) {.thread.} =
  ...
  {.cast(gcsafe).}:
    when defined(debug): stderr.writeLine("[SOCKS] reader started for slot=" & $idx & " server_id=" & $sServerId[idx])
  ...
  while sAlive[idx]:
    {.cast(gcsafe).}:
      try:
        ...
        if n <= 0:
          when defined(debug): stderr.writeLine("[SOCKS] reader EOF/close for slot=" & $idx & " server_id=" & $sServerId[idx])
          sAlive[idx] = false
          break
        when defined(debug): stderr.writeLine("[SOCKS] reader got " & $n & " bytes from TCP slot=" & $idx & " server_id=" & $sServerId[idx])
        sOutChan[idx].send(buf[0 ..< n])
      except CatchableError as e:
        when defined(debug): stderr.writeLine("[SOCKS] reader error slot=" & $idx & ": " & e.msg)
        sAlive[idx] = false
        break
  ## Notify main loop that this connection is closed
  {.cast(gcsafe).}:
    when defined(debug): stderr.writeLine("[SOCKS] reader exiting slot=" & $idx & ", notifying main loop")
...
proc socksHandleData*(serverId: int, rawData: string,
  ...
  if i < 0:
    ...
    when defined(debug): stderr.writeLine("[SOCKS] new conn server_id=" & $serverId & " data_len=" & $rawData.len)
    ...
    if not ok:
      ...
      when defined(debug): stderr.writeLine("[SOCKS] CONNECT parse failed for server_id=" & $serverId)
      result.add((serverId, socks5ConnectReply(false)))
      sAlive[idx] = false
      return

    when defined(debug): stderr.writeLine("[SOCKS] CONNECT parsed: " & target.host & ":" & $target.port)
    ...
    try:
      ...
      when defined(debug): stderr.writeLine("[SOCKS] TCP connected to " & target.host & ":" & $target.port)
      ## Send success reply
      ...
      when defined(debug): stderr.writeLine("[SOCKS] TCP connect failed: " & e.msg)
      result.add((serverId, socks5ConnectReply(false)))
      sAlive[idx] = false
    return
  ...
  if sState[i] == scsRelay and rawData.len > 0:
    when defined(debug): stderr.writeLine("[SOCKS] relay " & $rawData.len & " bytes to TCP server_id=" & $serverId)
    try:
      sSocket[i].send(rawData)
    except CatchableError as e:
      when defined(debug): stderr.writeLine("[SOCKS] relay send failed: " & e.msg)
      sAlive[i] = false
